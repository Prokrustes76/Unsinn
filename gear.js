class Armor {
  ...
  getStats() {
    ...
    let values = [{cha: 'Armor' ,  val : 25 * this.level},
                  ...
                  {cha: 'Healing', val : 1 * this.level},
                  {cha: 'Damage',  val : 1 * this.level},
  ...
  show(pos = this.pos) {
    if (this.nr >= 60)
      ctx.drawImage(info.weapon, 9 + 91 * (this.picNr % 21), 3 + 90 * Math.floor(this.picNr / 21), 
                    85, 85,  pos.x, pos.y, pos.w, pos.h)  
    else
...
class Weapon {
  constructor(nr, pos) {
    this.nr           = nr
    this.level        = nr % 5
    this.part         = nr < 60 ? Weapon.parts[Math.floor((nr % 20) / 5)] : 'Offhand'
    this.material     = nr < 60 ? Weapon.material[Math.floor(nr / 20)]     : ''
    this.picNr        = this.getPicNr(nr)    
    this.name         = this.getName()
    this.rs           = this.calcRS()
    this.slotAmount   = Armor.getSlotAmount()
    this.gems         = []  
    this.pos          = pos
  }
